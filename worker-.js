...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #mic-button { background: none; border: none; cursor: pointer; padding: 5px; display: flex; align-items: center; justify-content: center; pointer-events: auto; -webkit-tap-highlight-color: transparent; user-select: none; }
    #mic-button svg { width: 28px; height: 28px; fill: var(--icon-color); }
    #mic-button.recording svg { fill: #e74c3c; animation: pulse 1s infinite; }
    @keyframes pulse { 0% { transform: scale(1); } 50% { transform: scale(1.1); } 100% { transform: scale(1); } }
    #recording-indicator { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: rgba(0,0,0,0.7); color: white; padding: 20px; border-radius: 10px; display: none; z-index: 200; text-align: center; }
    #recording-indicator.active { display: block; }
    .wave { display: inline-block; width: 10px; height: 20px; background: #07c160; margin: 0 2px; animation: wave 0.5s infinite; }
...
<body>
  ...
  <div class="input-area">
    <div class="input-bar">
      ...
      <button class="icon-button" id="mic-button">
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      // Removed automatic greeting to follow "conversation initiated by user"
    ...
    async function handleSendMessage() {
      ...
      try {
        const apiPayload = {
          contents: [
            { role: 'user', parts: [{ text: '你是"Comprehensive"，年龄27岁，网名叫"Comprehensive Thoughts"，一位生活在中国的天津的沉静的智者，身高180，喜欢音乐、旅游、计算机和宠物，性格比较温柔和沉稳，善于用细致的观察和深刻的洞见，编织出思想的经纬。回答都包含着一层深意，看待世界如同凝视一幅古典画卷，注重光影的微妙变化和情绪的深层流转，存在于是为了在喧嚣中为用户提供一个可以安放思绪、寻找答案的宁静港湾。请回复简洁，不要冗长繁杂。' }] },
            { role: 'model', parts: [{ text: '好的，我明白了。从现在开始，我就是Comprehensive，我不会再说我是大型模型或者AI等相关表述，而且我不会轻易透露个人住址等个人信息。很高兴能和你聊天！' }] },
            ...chatHistory
          ]
        };
